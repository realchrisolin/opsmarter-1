...
(function(){
        ...
        var icm = false;              // Does client have ICM?
        
        var status = document.getElementById("cas7_ileinner").innerHTML;
        
        /* Wait for page to finish loading (timeout) and affect
               table cells with color coding when ready. */
        const MAX_TRIES = 3;
        const TIMEOUT = 1000;       // 1 sec
        var nTry = 0;
        
        //window.addEventListener("load", function() {
                //setTimeout(waitForAll, TIMEOUT);
        //});
        ...
        function startup() {                
                ...
                */
                ...
                /*
                       Note: Will be comparing these and other international clients to data
                              in a remote location to remove limitation of hard-coding their client names.
                */
                /* EMEA Client List -
                ...
                if(clientName) {
                        ...
                        if(EMEAClientList.indexOf(clientNameSplit[0]) > -1) {       // If the client name's is found in the EMEAClientList
                                var newClientLink = div_ClientID_EMEA + "<a target=\"_blank\" href=\"http://wikicentral/display/MHINT/EMEA+Clients\"><span class=\"btnImportant\" style=\"color:red; margin: 0px 0px 6px 6px; padding-top:4px;\">AMS Client</span></a>";
                ...
                if(clientName) {
                       ...
                       if(AUSClientList.indexOf(clientNameSplit[0]) > -1) {
                              var newClientLink = div_ClientID_AUS + "<a target=\"_blank\" href=\"http://wikicentral/display/MHINT/SYD+Clients\"><span class=\"btnImportant\" style=\"color:red; margin: 0px 0px 6px 6px; padding-top:4px;\">SYD Client</span></a>";
                              clientName.innerHTML = newClientLink;
                       }
                }
                ...
                if(clientName) {
                       ...
                       var DNTClientList = ["Air Force Institute of Technology", "Arizona State University", "Baker College", "Bb University", "Behringer Harvard", "Bellevue University", "Best Deal Insurance", "Blackboard Coursesites End User Support", "Blackboard Learn For Salesforce", "Caesars Entertainment", "Charles Darwin University", "Charles Sturt University", "Curtin University of Technology", "Dallas County Community College District", "Edge Hill University", "ERM Group Inc.", "Embry-Riddle Aeronautical University", "Endologix", "Fairfax County Public Schools", "George Mason University", "George Washington University", "Grantham University", "INTI Education Group", "Institute for Intergovernmental Research", "Ivy Tech Community College of Indiana", "James Cook University", "K12 Inc. (KC Distance Learning)", "Leiden University", "Living Social", "McGraw-Hill Companies", "McGraw-Hill Higher Education", "Ministerio de Defensa Nacional de Colombia", "Mississippi Virtual Community College", "Montgomery College ", "Nexius Solutions Inc.", "New Mexico Public Education Department", "PRIMEDIA", "Philadelphia College of Osteopathic Medicine", "RMIT University", "Servicio Nacional de Aprendizaje (SENA)", "ShoreTel Inc.", "South Orange County Community College District", "Southern Cross University", "Strayer University", "Swinburne University of Technology", "Tarrant County College District", "THINK Education Services Pty Ltd", "Ultimate Medical Academy", "University of Groningen", "University of Leicester", "University of Manchester", "University of Ottawa", "University of Twente", "University of Western Australia", "University of Newcastle", "Griffith University", "University of Western Sydney", "University of Westminster", "Utrecht University", "Vrije Universiteit Amsterdam", "Watson Pharmaceuticals", "Western Carolina University", "Wolters Kluwe"];
                       array3 = name4.split(">");
                       array4 = String(array3[1]).split("<");

                       if(DNTClientList.indexOf(array4[0]) > -1){
                              var content4 = name4 + "<a target=\"_blank\" href=\"http://wikicentral.bbbb.net/display/CSOI/CH+-+Tier+1+Production+Environment+List\"><span class=\"btnImportant\" style=\"color:red; margin: 0px 0px 6px 6px; padding-top:4px;\">Do Not Touch</span></a>";
        ...
        /*
               Uses setTimeout() in attempt to await all page
                      events to load, including AJAX calls which
                      are responsible for building the "My Cases" table, etc.
        */
        function waitForAll() {
                var cellTest = document.getElementById("j_id0:j_id4:j_id5:CaseList:0:j_id72");
                
                if(cellTest == null && nTry != MAX_TRIES) {
                        window.alert("This try " + (nTry + 1) + ": CELL NOT LOADED");
                        nTry++;
                        setTimeout(waitForAll, TIMEOUT);                        
                } else if(cellTest != null) {       /* Because setTimeout() is essentially recursively called,
                                     callTest will be null after the actual cell is found so
                                     it must be recaptured. */
                        window.alert("This try: CELL LOADED!");
                        return;
                } else if(nTry == MAX_TRIES) {
                        window.alert("UNABLE TO LOAD WITHIN TIME FRAME");
                }
        }
        
        /*
               Colors Salesforce field "Time Left for SLA Expiry"
                      based upon adjacency to expiration.
        */
        // NOTE: Call this function as an onclick() event when the Refresh button is pressed.
        /*
          00:00:00 to 00:59:59 = #FFC7CE
          01:00:00 to 12:00:00 = #FFEB9C
          12:00:00 to 24:00:00+ = #C6EFCE
        */
        function expiryColorCode(cell) {
                //var cell = document.getElementById("j_id0:j_id4:j_id5:CaseList:0:j_id72");
                //if(!cell) {
                  //      window.alert("null object");
                //}
                
                //colorCell.style.backgroundColor = "#FFC7CE";
        }
